...
| 內容類型 | 你該信誰 |
|---|---|
| 網頁上的按鈕在哪、選單叫什麼、畫面長怎樣 | 🔴 **信你眼前的畫面，不要信這份文件。** Supabase 的 UI 一直在改 |
| 官方 API 的用法、參數、最新做法 | 🔴 **去查官方文件**（上方已列來源），不要靠記憶 |
| **紅線**（哪些事會害到學生資料） | ✅ **信這份**，這是使用者的要求，官方文件不會寫 |
| **陷阱**（不會自己說出來的預設行為） | ✅ **信這份**，這是踩過才知道的 |
| **症狀 → 真因對照** | ✅ **信這份**，這是最省時間的部分 |

### 三條執行原則

1. **先查現況再動手。** 動任何設定之前，先跑指令看這台目前是什麼狀態，並回報給使用者。
2. **失敗時先報錯誤原文，然後停下來。** 不要自己換方法重試——**尤其不要換憑證**。
   今天現場的混亂就是這樣來的：一個 `project_ref` 打錯，agent 一路換金鑰亂試了一個下午。
3. **文件跟現況對不上時，以現況為準，並告訴使用者哪裡對不上。** 不要硬套這份文件的字面步驟。
...
開 <https://supabase.com> 自己註冊，建議用 **Continue with GitHub**（跟 `07-github` 同一個帳號，之後好管理）。

> 按鈕文字與版面以**你眼前的畫面**為準，不要照這份文件找字。

### 1-2 建立專案

建新專案時，這四個欄位要注意（**欄位名稱可能微調，看語意對應**）：
