...
**這份的價值不在「步驟」，在「你查不到的那些事」。**

| 內容類型 | 你該信誰 |
|---|---|
| `gh` 的指令名稱、旗標、輸出格式 | 🔴 **信 `--help` 與官方文件**。gh 版本一直在更新 |
| GitHub 網頁上的按鈕在哪、選單叫什麼 | 🔴 **信使用者眼前的畫面** |
| **「兩道門」的觀念**（Git 協定門 vs API 門） | ✅ **信這份**。這解釋了九成的「SSH key 明明設好了卻建不了 repo」 |
| **裝置碼會被你的逾時砍掉** | ✅ **信這份**。實測過，退出碼 124 |
| **註冊拼圖是風險評分不是解題** | ✅ **信這份**。同 IP 多人註冊會被判機器人 |
| **credential helper 查不到 ≠ 沒設好** | ✅ **信這份**。GCM 的設定在 system 層 |
| **症狀 → 真因對照**（常見坑那張表） | ✅ **信這份**，最省時間的部分 |

### 三條執行原則

1. **先查現況再動手。** 先跑 `gh auth status`、`git config --show-origin --get-all credential.helper` 看這台是什麼狀態，回報給使用者。
2. **失敗時先報錯誤原文，然後停下來。** 不要自己換方法重試，**尤其不要重跑 `gh auth login`**——重跑一次就作廢一組碼，使用者會愈弄愈亂。
3. **文件跟現況對不上時，以現況為準**，並告訴使用者哪裡對不上。

> 🔴 **這一包最容易搞砸的三件事，全部都是「你多做了什麼」而不是「你少做了什麼」**：
> 重跑 `gh auth login`、在自己的 shell 跑互動指令、看到 `--global` 查不到就叫使用者去修。
> **看到卡住，先停、先問，不要動手。**
...
- ✅ **註冊完順便在瀏覽器登入一次**（如果會跳信箱驗證、裝置確認、2FA 設定，一併做完）
- ❌ 不要全班在現場一起註冊

> 💡 **第二點比看起來重要。** 隔天現場要跑 `gh auth login`，如果瀏覽器已經是登入狀態，
> 老師只剩「打八碼、按 Authorize」兩個動作，幾十秒的事。
> 反之，如果他是當天才第一次登入，中間會夾著信箱驗證、裝置確認、甚至被要求當場設 2FA——
> **這些全部要塞進授權的時間裡**，就是現場「重複三次認證才成功」的由來（見 2-2）。
...
#### 輪詢節奏：每 10 秒查一次，**最多等 10 分鐘**

**不要只等 5 分鐘。** 現場實際發生過「重複三次認證才成功」——那不是重試三次終於通了，
是**第三次才跑贏逾時**。第一次還在找瀏覽器、第二次還在登入，第三次熟了才來得及。

**等待期間每分鐘回報一次**，例如：

> 「還在等你完成授權，不用急。完成之後我這邊會自己偵測到。」

**這句話很重要。** 沒有回報的話，老師會以為當掉了，跑去自己重跑 `gh auth login`——
**那會作廢目前這組碼**，等於親手把自己的進度歸零，然後又要重來一輪。

10 分鐘還沒好，才問他「你那邊卡在哪一步？」——**還是不要自己重跑**。
...
| **當天現場才註冊新帳號的人，登入特別慢** | 全新帳號的第一次登入不只是打八碼，中間常常還夾著<br>**信箱驗證** ＋ **首次登入的裝置確認** ＋ 有些人會被要求**當場設 2FA**。<br>這些全部要塞進授權的時間裡，**比老手慢好幾倍是正常的**。<br>→ 輪詢一律給滿 10 分鐘，並每分鐘回報一次讓他安心。<br>→ **根本解法還是前一晚在家裡先把帳號註冊好、順便登入一次**（見 2-1）——<br>　 隔天現場就只剩「打八碼、按 Authorize」兩個動作，幾十秒的事 |
